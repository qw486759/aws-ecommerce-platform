...
resource "aws_ecs_task_definition" "app" {
  ...
  tags = { Name = "ecommerce-task" }
}

resource "aws_security_group" "ecs_tasks" {
  name        = "ecommerce-ecs-tasks-sg"
  ...
  tags = { Name = "ecommerce-ecs-tasks-sg" }
}
...
resource "aws_ecs_cluster" "staging" {
  ...
  tags = { Name = "ecommerce-staging-cluster" }
}
...
resource "aws_ecs_task_definition" "app_staging" {
  ...
  container_definitions = jsonencode([{
    name      = "ecommerce-app"
    image     = "${aws_ecr_repository.app.repository_url}:staging"
    essential = true
  ...
  tags = { Name = "ecommerce-task-staging" }
}

# ── Staging: ECS Tasks Security Group ────────────────────────────────────────
resource "aws_security_group" "ecs_tasks_staging" {
  name        = "ecommerce-ecs-tasks-staging-sg"
  description = "Allow traffic from ALB to staging ECS tasks"
  vpc_id      = data.aws_vpc.main.id
  ...
  tags = { Name = "ecommerce-ecs-tasks-staging-sg" }
}

# ── Staging: ECS Service ──────────────────────────────────────────────────────
resource "aws_ecs_service" "staging" {
  name            = "ecommerce-staging-service"
  ...
  tags = { Name = "ecommerce-staging-service" }
}